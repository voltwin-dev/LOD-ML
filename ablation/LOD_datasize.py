...
from utils.dataset import PDEBenchDataset_Datasize

# Import models
from models.LOD import LOD, LOD_wo_PE
...
def run_training(config):
    ...
    N_eigen = config.pod_parameter.N_eigen # 64
    data_num = config.pod_parameter.data_num
    ...
    train_data = PDEBenchDataset_Datasize(file_names,
    ...
    val_data = PDEBenchDataset_Datasize(test_file_names,
                            reduced_resolution=reduced_resolution,
                            reduced_resolution_t=reduced_resolution_t,
                            reduced_batch=reduced_batch,
                            initial_step=initial_step,
    ...
    elif model_name == 'lod_wo_PE':
        print("LOD")
        model = LOD_wo_PE(init_t=initial_step*num_channels,
                    N_eigen=N_eigen,
                    N_time=t_train*num_channels,
                    N_x=x_range,
                    bases=bases[0],
    ...
    model_path = "./checkpoint/abl_" + model_name + "_" + config.dataset.name + '_size_'+ str(data_num) +'.pt'
    ...
    for ep in tqdm(range(epochs)):
        ...
        for xx, yy, grid, pde_param, coeff, bases in train_loader:
            ...
            #####
            elif (model_name == 'lod') or (model_name == 'lod_wo_PE'):
                # Reshape input tensor into [b, x1, ..., xd, t_init*v]
                inp = xx.reshape(inp_shape)

                y = yy[..., :t_train, :] # (N, S, T, 1)

                pred_pde, latent_pde, pred_coeff, latent_coeff = model(inp)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pde', default='abl_datasize')
